...
} from "@/services/playerProfileService"; // сервис анкеты игрока
...
const getTeamStatusBadge = (status: "looking" | "has-team" | "not-looking") => {
  switch (status) {
    case "looking":
      return <Badge className="bg-green-500 text-white">Ищет команду</Badge>;
    case "has-team":
...
const PlayerProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchPlayerProfile = async () => {
      ...
      try {
        ...
        setError(
          "Не удалось загрузить профиль игрока. Возможно, он был удален или ссылка неверна."
        );
        toast({
  ...
  const handleInvitePlayer = async () => {
    ...
    try {
      ...
      let errorMessage =
        "Произошла неизвестная ошибка при отправке приглашения.";
              ...
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-1 text-gray-600 mt-2">
                    ...
                    <span>
                      {player.location || "Местоположение не указано"}
                    </span>
                  </div>
                  <div className="mt-4 flex flex-wrap items-center gap-2">
                    {/* Статус поиска команды теперь здесь */}
                    {getTeamStatusBadge(
                      mapTeamFindingStatusToFrontend(player.teamFindingStatus)
                    )}
                    ...
                    <div className="flex items-center gap-2 p-2 border rounded-md">
                      ...
                      <Badge variant="outline">
                        {player.game || "Не указан"}
                      </Badge>
                      <Badge variant="secondary">
                        {mapSkillLevelToExperience(player.skillLevel)}
                      </Badge>
                    </div>

                    {/* Второй вид спорта, если он есть */}
                    {player.game2 && (
                      <div className="flex items-center gap-2 p-2 border rounded-md bg-sky-50 border-sky-200">
                        <Gamepad2 className="h-4 w-4 text-sky-700" />
                        <Badge variant="outline" className="bg-white">
                          {player.game2}
                        </Badge>
                        <Badge className="bg-sky-200 text-sky-800 hover:bg-sky-300">
                          {mapSkillLevelToExperience(player.skillLevel2)}
                        </Badge>
                      </div>
                    )}
                  </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              <p>
                <strong>Email:</strong> {player.email || "не указан"}
              </p>
              <p>
                <strong>Телефон:</strong> {player.phone || "не указан"}
              </p>
              <p>
                <strong>Telegram:</strong> {player.telegram || "не указан"}
              </p>
            </CardContent>
          </Card>
        </div>
